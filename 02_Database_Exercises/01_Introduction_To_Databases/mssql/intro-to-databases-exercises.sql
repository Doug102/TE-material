...
SELECT name, continent, population, gnp, lifeexpectancy 'average life expectancy'
FROM country
...
SELECT name, continent, headofstate 'head of state'
FROM country
...
SELECT name, region
FROM country
WHERE region != 'Caribbean' AND continent = 'North America' OR continent = 'South America' ;
...
SELECT name, lifeexpectancy 'average life expectancy'
FROM country
...
SELECT name, continent, gnp, lifeexpectancy 'average life expectancy'
FROM country
...
SELECT (gnp * 1000000) / population 'per capita GNP', name
FROM country
WHERE continent = 'Europe';
-- 22. The number of years since independence for all countries that have a year of independence (192 rows)
SELECT 2021 - indepyear 'number of years since independence', name
FROM country
