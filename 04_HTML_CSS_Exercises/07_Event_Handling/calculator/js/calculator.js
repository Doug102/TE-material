...
document.addEventListener('DOMContentLoaded', () => {
  ...
  numbers.forEach((number) => {
    number.addEventListener('click', (eventObject) => {
      clickNumber(eventObject);
    });
  });
  ...
  decimal.addEventListener('click', (eventObject) => {
    clickNumber(eventObject);
  });
  ...
  operators.forEach((operator) => {
    operator.addEventListener('click', (eventObject) => {
      clickOperator(eventObject);
    });
  });
  // add click event listener for the equal sign
  // should call the function performOperation
  const equalSign = document.querySelector('.equal-sign');
  equalSign.addEventListener('click', (eventObject) => {
    performOperation();
  });
});
